<template>
    <v-app id="app-content">
        <v-navigation-drawer :mini-variant="showMenu || isMini" app clipped :permanent="showMenu">
            <v-list>
                <router-link class="d-block text-decoration-none" :to="{ name: 'empleados' }">
                ...
                <label class="py-2 pl-4 d-block itb-drawer-label mt-3" v-if="$store.state.profile.role === 'admin'">
                    <v-icon v-if="showMenu || isMini">mdi-cog-outline</v-icon>
                    <span v-else>{{ $t("title.settings") }}</span>
                </label>

                <router-link class="d-block text-decoration-none" :to="{ name: 'users' }"
        ...
        <v-app-bar app clipped-left dark color="primary">
            ...
            <v-tooltip bottom v-if="$store.state.profile.role === 'admin'">
                <template v-slot:activator="{ on, attrs }">
                    <v-icon v-bind="attrs" v-on="on" color="yellow"
                        class="mr-2 itb-icon-tooltip">mdi-crown-outline</v-icon>
                </template>
            ...
            <v-spacer></v-spacer>
            <div>
                <v-menu v-model="isUserInfoVisible" :close-on-content-click="false" :nudge-width="200" offset-x>
                    <template v-slot:activator="{ on: onMenu, attrs: attrsMenu }">
                        <v-tooltip bottom>
                            <template v-slot:activator="{
                                ...
                                <v-btn v-bind="{ ...attrsMenu, ...attrsTooltip }" v-on="{ ...onMenu, ...onTooltip }"
                                    icon color="grey darken-1" class="text-decoration-none">
                                    <v-avatar color="grey darken-1" size="40">
                                        <img v-if="$store.state.profile.avatar" :src="'/uploads/' +
                                            $store.state.profile.avatar
                                            " :alt="$store.state.profile.name +
                                                '-avatar'
                                                " />
                                        <span v-else class="white--text text-body">{{ userInitials }}</span>
                                    </v-avatar>
                                </v-btn>
                            </template>
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            <v-list two-line>
                                ...
                                <v-list-item v-if="$store.state.profile.country">
                                    <v-list-item-action></v-list-item-action>
                                ...
                                <v-list-item v-if="$store.state.profile.postalcode">
                                    <v-list-item-icon>
                                ...
                                <v-list-item v-if="$store.state.profile.address">
                                    <v-list-item-icon>
                                ...
                                <v-list-item v-if="$store.state.profile.company">
                                    <v-list-item-action></v-list-item-action>
            ...
            <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                    <v-btn icon v-bind="attrs" v-on="on" @click="logout" color="deep-orange accent-3">
        ...
        <OwnUserForm :user="selectedUser" :timezones="timezones" :languages="languages" :visible="dialog.ownUserForm"
            @updateUser="updateUser" @close="closeUserDialog" @snackMessage="snackMessage" />
